...
int OptimizationProblemInterface::finiteCombinedBoundsDimension()
{
    ...
    for (int i = 0; i < getParameterDimension(); ++i)
    {
        if (getLowerBound(i) > -MHP_PLANNER_INF_DBL || getUpperBound(i) < MHP_PLANNER_INF_DBL) ++dim;
    }
    return dim;
}
...
int OptimizationProblemInterface::finiteBoundsDimension()
{
    ...
    for (int i = 0; i < getParameterDimension(); ++i)
    {
        if (getLowerBound(i) > -MHP_PLANNER_INF_DBL) ++dim;
        if (getUpperBound(i) < MHP_PLANNER_INF_DBL) ++dim;
    }
    return dim;
}
...
void OptimizationProblemInterface::computeDistanceFiniteCombinedBounds(Eigen::Ref<Eigen::VectorXd> values)
{
    ...
    for (int i = 0; i < getParameterDimension(); ++i)
    {
        lb = getLowerBound(i);
        ub = getUpperBound(i);
        if (lb > -MHP_PLANNER_INF_DBL || ub < MHP_PLANNER_INF_DBL)
        {
                                                                                      ...
                                                                                      Eigen::Ref<Eigen::VectorXi> j_col)
{
    ...
    {
        // check if finite
        if (getLowerBound(i) > -MHP_PLANNER_INF_DBL || getUpperBound(i) < MHP_PLANNER_INF_DBL)
        {
                                                                            ...
                                                                            bool include_finite_bounds)
{
    ...
    if (dim_ineq > 0)
    {
        lbA.segment(dim_eq, dim_ineq).setConstant(-MHP_PLANNER_INF_DBL);  // should be smaller than -OSQP_INFTY
