...
{
    if (x_lb.size() == 0)
        x_lb.setConstant(x_dim, -MHP_PLANNER_INF_DBL);
    else if (x_lb.size() != x_dim)
        PRINT_ERROR_NAMED("Error in lower state bounds: dimensions mismatch");

    if (x_ub.size() == 0)
        x_ub.setConstant(x_dim, MHP_PLANNER_INF_DBL);
    else if (x_ub.size() != x_dim)
        PRINT_ERROR_NAMED("Error in upper state bounds: dimensions mismatch");

    if (u_lb.size() == 0)
        u_lb.setConstant(u_dim, -MHP_PLANNER_INF_DBL);
    else if (u_lb.size() != u_dim)
        PRINT_ERROR_NAMED("Error in lower control input bounds: dimensions mismatch");

    if (u_ub.size() == 0)
        u_ub.setConstant(u_dim, MHP_PLANNER_INF_DBL);
    else if (u_ub.size() != u_dim)
        PRINT_ERROR_NAMED("Error in upper control input bounds: dimensions mismatch");
}

void NlpFunctions::setMultistagePlannerId(const int id)
{
    stage_cost->setPlannerId(id);
    final_stage_cost->setPlannerId(id);
    stage_inequalities->setPlannerId(id);
}

std::vector<int> NlpFunctions::getMultistagePlannerId() const
{
    return std::vector<int>{stage_cost->getPlannerId(), final_stage_cost->getPlannerId(), stage_inequalities->getPlannerId()};
}

bool NlpFunctions::isMultistagePlannerSet() const
{
    bool isSet = true;
    isSet &= stage_cost->isPlannerSet();
    isSet &= final_stage_cost->isPlannerSet();
    isSet &= stage_inequalities->isPlannerSet();
