...
#ifndef SRC_CORE_INCLUDE_MHP_PLANNER_CORE_REFERENCE_TRAJECTORY_H_
#define SRC_CORE_INCLUDE_MHP_PLANNER_CORE_REFERENCE_TRAJECTORY_H_
...
class DiscreteTimeReferenceTrajectory : public ReferenceTrajectoryInterface
{
 public:
    ...
    DiscreteTimeReferenceTrajectory() = default;
    explicit DiscreteTimeReferenceTrajectory(const TimeSeries& trajectory, TimeSeries::Interpolation interpolation) : _interpolation(interpolation)
    {
    ...
    bool isStatic() const override { return _trajectory.getTimeDimension() == 1; }
    bool isZero() const override
    {
        if (_trajectory.getValueDimension() == 0) return false;

        return std::all_of(_trajectory.getValues().begin(), _trajectory.getValues().end(), [](double i) { return (i < 1e-9 && i > -1e-9); });
    }
    int getDimension() const override { return _trajectory.getValueDimension(); }
    ...
    void getReference(const Time& t, OutputVector& ref) const override
    {
        if (_trajectory.getValueDimension() == 0)
        {
    ...
    void setTrajectory(const TimeSeries& trajectory, TimeSeries::Interpolation interpolation)
    {
        setTrajectory(trajectory);
        _interpolation = interpolation;
    }

    void setTrajectory(const TimeSeries& trajectory)
    {
        if (trajectory.getValueDimension() == 0)
        {
    ...
    void setTimeFromStart(const Time& time_from_start) { _trajectory.setTimeFromStart(time_from_start.toSec()); }

    void setInterpolationMethod(TimeSeries::Interpolation interpolation) { _interpolation = interpolation; }

    const TimeSeries& getReferenceTrajectory() { return _trajectory; }

 private:
    TimeSeries _trajectory;
    std::vector<OutputVector> _cached_trajectory;
    std::vector<double> _cached_dt;
    Time _cached_t;
...
class BlindDiscreteTimeReferenceTrajectory : public ReferenceTrajectoryInterface
{
 public:
    ...
    BlindDiscreteTimeReferenceTrajectory() = default;
    explicit BlindDiscreteTimeReferenceTrajectory(const TimeSeries& trajectory, TimeSeries::Interpolation interpolation)
        : _interpolation(interpolation)
    {
        setTrajectory(trajectory);
    }
...
#endif  // SRC_CORE_INCLUDE_MHP_PLANNER_CORE_REFERENCE_TRAJECTORY_H_
