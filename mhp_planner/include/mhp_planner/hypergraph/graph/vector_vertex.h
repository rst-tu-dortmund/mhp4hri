...
#ifndef SRC_OPTIMIZATION_INCLUDE_MHP_PLANNER_OPTIMIZATION_HYPER_GRAPH_VECTOR_VERTEX_H_
#define SRC_OPTIMIZATION_INCLUDE_MHP_PLANNER_OPTIMIZATION_HYPER_GRAPH_VECTOR_VERTEX_H_

#include <mhp_planner/hypergraph/graph/vertex_interface.h>
 ...
 public:
    ...
    explicit VectorVertex(int dimension, bool fixed = false)
        : _values(Eigen::VectorXd::Zero(dimension)),
          _lb(Eigen::VectorXd::Constant(dimension, -MHP_PLANNER_INF_DBL)),
          _ub(Eigen::VectorXd::Constant(dimension, MHP_PLANNER_INF_DBL)),
    ...
    explicit VectorVertex(const Eigen::Ref<const Eigen::VectorXd>& values, bool fixed = false)
        : _values(values),
          _lb(Eigen::VectorXd::Constant(values.size(), -MHP_PLANNER_INF_DBL)),
          _ub(Eigen::VectorXd::Constant(values.size(), MHP_PLANNER_INF_DBL)),
          _finite_lb_bounds(false),
          _finite_ub_bounds(false),
          _fixed(fixed)
    {
    ...
    virtual void setDimension(int dim)
    {
        ...
        _lb               = Eigen::VectorXd::Constant(dim, -MHP_PLANNER_INF_DBL);
        _ub               = Eigen::VectorXd::Constant(dim, MHP_PLANNER_INF_DBL);
    ...
    {
        _lb               = lb;
        _finite_lb_bounds = (_lb.array() > -MHP_PLANNER_INF_DBL).any();
    }
    // implements interface method
    ...
    {
        _ub               = ub;
        _finite_ub_bounds = (_ub.array() < MHP_PLANNER_INF_DBL).any();
    }
    // implements interface method
    ...
    {
        assert(idx < _lb.size());
        return _lb[idx] > -MHP_PLANNER_INF_DBL;
    }
    // implements interface method
    bool hasFiniteUpperBound(int idx) const override
    {
        assert(idx < _lb.size());
        return _ub[idx] < MHP_PLANNER_INF_DBL;
    }
    // implements interface method
    int getNumberFiniteLowerBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _fixed)
            ...
            return (_lb.array() > -MHP_PLANNER_INF_DBL).count();
    }
    // implements interface method
    int getNumberFiniteUpperBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _fixed)
            ...
            return (_ub.array() < MHP_PLANNER_INF_DBL).count();
    }
    int getNumberFiniteBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _fixed)
            ...
            return (_ub.array() < MHP_PLANNER_INF_DBL || _lb.array() > -MHP_PLANNER_INF_DBL).count();
    }
 ...
 public:
    ...
    int getNumberFiniteLowerBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _num_unfixed > 0)
        {
            ...
            for (int i = 0; i < getDimension(); ++i)
            {
                if (!_fixed[i] && _lb[i] > -MHP_PLANNER_INF_DBL) num += 1;
            }
            return num;
        }
        else
    ...
    int getNumberFiniteUpperBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _num_unfixed > 0)
        {
            ...
            for (int i = 0; i < getDimension(); ++i)
            {
                if (!_fixed[i] && _ub[i] < MHP_PLANNER_INF_DBL) num += 1;
            }
            return num;
        }
        else
    ...
    int getNumberFiniteBounds(bool unfixed_only) const override
    {
        if (unfixed_only && _num_unfixed > 0)
        {
            ...
            for (int i = 0; i < getDimension(); ++i)
            {
                if (!_fixed[i] && (_ub[i] < MHP_PLANNER_INF_DBL || _lb[i] > -MHP_PLANNER_INF_DBL)) num += 1;
            }
            return num;
        }
        else
...
#endif  // SRC_OPTIMIZATION_INCLUDE_MHP_PLANNER_OPTIMIZATION_HYPER_GRAPH_VECTOR_VERTEX_H_
