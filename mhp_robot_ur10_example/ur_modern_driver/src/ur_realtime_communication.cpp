...
void UrRealtimeCommunication::run()
{
    ...
    while (keepalive_)
    {
        while (connected_ && keepalive_)
        {
            ...
            if (bytes_read > 0)
            {
                ...
                if (safety_count_ == safety_count_max_)
                {
                    ...
                    print_warning("Low level timeout.");
                }
                safety_count_ += 1;
            }
            else
        ...
        if (keepalive_)
        {
            // reconnect
            print_warning(
                "Realtime port: No connection. Is controller crashed? Will "
            ...
            while (keepalive_ && !connected_)
            {
                ...
                {
                    print_error(
                        "Error re-connecting to RT port 30003. Is controller "
                        "started? Will try to reconnect in 10 seconds...");
                }
                else
                {
                    connected_ = true;
...
void UrRealtimeCommunication::setSafetyCountMax(uint inp) { safety_count_max_ = inp; }
