...
class RosWrapper
{
 protected:
    ...
    std::thread* rate_transition_thread_;
    std::vector<double> joint_speeds_;
    double acc_;
    boost::shared_ptr<ros_control_ur::UrHardwareInterface> hardware_interface_;
    boost::shared_ptr<controller_manager::ControllerManager> controller_manager_;
    std::mutex rate_transition_mutex_;
    int speed_callback_counter_;
    bool stopped_speed_loop_;
    bool first_joint_msg_;
    URUtility ur_utility_;
 ...
 public:
    RosWrapper(std::string host, int reverse_port)
        : robot_(rt_msg_cond_, msg_cond_, host, reverse_port, 0.03, 300),
          ...
          joint_speeds_(6, 0),
          acc_(15),
          speed_callback_counter_(0),
          stopped_speed_loop_(true),
          first_joint_msg_(true)
    {
        ...
        if (ur_utility_.getJointsCount() != 6)
        {
            print_error("Only 6 active joints in URDF are allowed!");
            halt();
        }

        for (int i = 0; i < ur_utility_.getJointsCount(); ++i)
        {
            joint_names.push_back(joint_prefix + ur_utility_.getJointNames()[i]);
        }
        ...
        if (use_ros_control_)
        {
            ...
            ros::param::get("~max_acceleration", acc_);
            acc_ = std::max(std::min(15.0, acc_), 1.0);

            sprintf(buf, "Max acceleration set to: %f [rad/sec²]", acc_);
            print_debug(buf);

            hardware_interface_->setMaxVelChange(acc_ / 125);  // = 15/125 = 0.12 equivalent of an acceleration of 15 rad/sec^2
        }
        ...
        tool_frame_ = joint_prefix + "ee_link";
        ...
        if (robot_.start())
        {
            ...
            rate_transition_thread_ = new std::thread(boost::bind(&RosWrapper::rateTransition, this));

            speed_sub_    = nh_.subscribe("joint_speed", 1, &RosWrapper::speedInterface, this);
            urscript_sub_ = nh_.subscribe("URScript", 1, &RosWrapper::urscriptInterface, this);

            io_srv_      = nh_.advertiseService("set_io", &RosWrapper::setIO, this);
            payload_srv_ = nh_.advertiseService("set_payload", &RosWrapper::setPayload, this);
        }
    }
 ...
 private:
    ...
    void goalCB(actionlib::ServerGoalHandle<control_msgs::FollowJointTrajectoryAction> gh)
    {
        ...
        if (!robot_.sec_interface_->robot_state_->isReady())
        {
            ...
            result_.error_string =
                "Cannot accept new trajectories. (Debug: Robot mode is " + std::to_string(robot_.sec_interface_->robot_state_->getRobotMode()) + ")";
            gh.setRejected(result_, result_.error_string);
            print_error(result_.error_string);
            return;
        }
        ...
        goal_handle_ = gh;
        if (!validateJointNames((*goal_handle_.getGoal()).trajectory.joint_names))
        {
    ...
    bool validateJointNames(const std::vector<std::string>& joint_names)
    {
        std::vector<std::string> actual_joint_names = robot_.getJointNames();
        if (joint_names.size() != actual_joint_names.size()) return false;
        ...
        for (unsigned int i = 0; i < joint_names.size(); i++)
        {
            ...
            {
                if (joint_names[i] == actual_joint_names[j]) break;
            }
            if (joint_names[i] == actual_joint_names[j])
            {
    ...
    void reorder_traj_joints(trajectory_msgs::JointTrajectory& traj)
    {
        ...
        for (unsigned int i = 0; i < traj.joint_names.size(); i++)
        {
            ...
            {
                if (traj.joint_names[i] == actual_joint_names[j]) mapping[j] = i;
            }
        }
        ...
        for (unsigned int i = 0; i < traj.points.size(); i++)
        {
            ...
            for (unsigned int j = 0; j < traj.points[i].positions.size(); j++)
            {
                ...
                if (traj.points[i].accelerations.size() != 0) new_point.accelerations.push_back(traj.points[i].accelerations[mapping[j]]);
            }
    ...
    bool has_velocities()
    {
        ...
        for (unsigned int i = 0; i < goal.trajectory.points.size(); i++)
        {
            if (goal.trajectory.points[i].positions.size() != goal.trajectory.points[i].velocities.size()) return false;
        }
        return true;
    }
    ...
    {
        actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::Goal goal = *goal_handle_.getGoal();
        if (goal.trajectory.points.size() == 0) return false;
        for (unsigned int i = 0; i < goal.trajectory.points.size(); i++)
        {
            if (goal.trajectory.points[i].positions.size() != goal.trajectory.joint_names.size()) return false;
        }
        return true;
    }
    ...
    bool has_limited_velocities()
    {
        ...
        for (unsigned int i = 0; i < goal.trajectory.points.size(); i++)
        {
            ...
            {
                if (fabs(goal.trajectory.points[i].velocities[j]) > max_velocity_) return false;
            }
        }
        return true;
    }
    ...
    bool traj_is_finite()
    {
        ...
        for (unsigned int i = 0; i < goal.trajectory.points.size(); i++)
        {
            for (unsigned int j = 0; j < goal.trajectory.points[i].velocities.size(); j++)
            {
                if (!std::isfinite(goal.trajectory.points[i].positions[j])) return false;
                if (!std::isfinite(goal.trajectory.points[i].velocities[j])) return false;
            }
        }
        return true;
    }
    ...
    void speedInterface(const trajectory_msgs::JointTrajectory::Ptr& msg)
    {
        if (msg->points[0].velocities.size() == 6)
        {
            if (first_joint_msg_)
            {
                if (!ur_utility_.initJointMapping(msg->joint_names))
                {
                    return;
                }
            ...
            if (msg->points[0].accelerations.size() > 0)
                acc_ = std::max(std::min(15.0, *std::max_element(msg->points[0].accelerations.begin(), msg->points[0].accelerations.end())), 1.0);

            // Update global joint speed variable
            rate_transition_mutex_.lock();
            ur_utility_.parseJointStates(joint_speeds_, msg->points[0].velocities);
            speed_callback_counter_ = 0;
            stopped_speed_loop_     = false;
            rate_transition_mutex_.unlock();
        }
        else
        {
            ROS_ERROR("Invalid joint speed message.");
        }
    }

    void urscriptInterface(const std_msgs::String::ConstPtr& msg) { robot_.rt_interface_->addCommandToQueue(msg->data); }
    ...
    void rateTransition()
    {
        ...
        while (ros::ok())
        {
            ...
            if (hardware_interface_->isSelfCollision() || stopped_speed_loop_)
            {
                continue;
            }

            rate_transition_mutex_.try_lock();

            if (speed_callback_counter_ > 25)
            {
                hardware_interface_->softStop();

                ROS_WARN("Joint Speed Timeout");
                stopped_speed_loop_ = true;
            }
            else
            {
                hardware_interface_->setSpeed(joint_speeds_[0], joint_speeds_[1], joint_speeds_[2], joint_speeds_[3], joint_speeds_[4],
                                              joint_speeds_[5], acc_);
                speed_callback_counter_ += 1;
            }
            rate_transition_mutex_.unlock();
    ...
    void rosControlLoop()
    {
        ...
        while (ros::ok())
        {
            ...
            if (hardware_interface_->isSelfCollision())
            {
                continue;
            }
            ...
            // Tool vector: Actual Cartesian coordinates of the tool: (x,y,z,rx,ry,rz), where rx, ry and rz is a rotation vector representation of the
            // tool orientation
    ...
    {
        bool warned           = false;
        ros::Publisher io_pub = nh_.advertise<ur_msgs::IOStates>("io_states", 1);
        ...
        while (ros::ok())
        {
            ...
            int i_max = 10;
            if (robot_.sec_interface_->robot_state_->getVersion() > 3.0) i_max = 18;  // From version 3.0, there are up to 18 inputs and outputs
...
int main(int argc, char** argv)
{
    ...
    if (!(ros::param::get("~robot_ip_address", host)))
    {
        ...
        {
            print_fatal("Could not get robot ip. Please supply it on the parameter server as robot_ip");
            exit(1);
        }
    }
